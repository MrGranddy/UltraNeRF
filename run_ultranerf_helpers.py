            ...
            for p_fn in self.kwargs['periodic_fns']:
                if B is not None:
                    embed_fns.append(lambda x, p_fn=p_fn,
                                 freq=freq, B=B: p_fn(x @ torch.transpose(B, 0, 1) * freq))
                    out_dim += d
                    out_dim += B.shape[1]
...
class NeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=6, skips=[4]):
        """ 
        """
...
def get_rays_us_linear(H, W, sw, sh, c2w):
    ...
    dirs_base = torch.tensor([0., 1., 0.], dtype=torch.float32, device=c2w.device)
    dirs_r = R @ dirs_base
