...
def compare_models():
    ...
    for scenario in scenarios:
        ...
        if scenario['use_viewdirs']:
            torch_model.bottleneck_layer.weight = nn.Parameter(torch.tensor(tf_model.layers[11].get_weights()[0].T))
            torch_model.bottleneck_layer.bias = nn.Parameter(torch.tensor(tf_model.layers[11].get_weights()[1]))
            ...
            torch_model.output_layers[4].weight = nn.Parameter(torch.tensor(tf_model.layers[17].get_weights()[0].T))
            torch_model.output_layers[4].bias = nn.Parameter(torch.tensor(tf_model.layers[17].get_weights()[1]))

            torch_model.alpha_layer.weight = nn.Parameter(torch.tensor(tf_model.layers[18].get_weights()[0].T))
            torch_model.alpha_layer.bias = nn.Parameter(torch.tensor(tf_model.layers[18].get_weights()[1]))

        else:
            torch_model.output_layers[0].weight = nn.Parameter(torch.tensor(tf_model.layers[11].get_weights()[0].T))
            torch_model.output_layers[0].bias = nn.Parameter(torch.tensor(tf_model.layers[11].get_weights()[1]))
